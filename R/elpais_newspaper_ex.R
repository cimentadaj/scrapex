##' This example hosts data from the website https://english.elpais.com/, which is
##' the main page of the newspaper El País.
##'
##' This function returns the full HTML link to the local HTML files that can be used as exercises for scraping the data as if this was the online hosted version.
##'
...
##'
##' @title Return the paths to the complete HTML website of the main page of newspaper El País.
...
##' }
##'
elpais_newspaper_ex <- function() {
  pkg_dir <- system.file(package = "scrapex")
  dir_data <- file.path(pkg_dir, "extdata", "elpais_newspaper/")

  html_files <-
    list.files(dir_data,
               pattern = "elpais_.+\\.html$",
               recursive = TRUE,
               full.names = TRUE)
